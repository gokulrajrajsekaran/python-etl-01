...
def load_incremental_orderdetails():
    conn = get_connection()
    cur = conn.cursor()
    ...
    try:
        # ==============================
        # Step 1: UPDATE existing orderdetails
        # ==============================
        update_sql = f"""
        ...
        print("Updated existing orderdetails where data changed.")
        ...
        LEFT JOIN {DEVDW_SCHEMA}.orders o
          ON o.src_orderNumber = s.orderNumber
        LEFT JOIN {DEVDW_SCHEMA}.products p
        ...
        conn.commit()
        print("Inserted new orderdetails records successfully.")
